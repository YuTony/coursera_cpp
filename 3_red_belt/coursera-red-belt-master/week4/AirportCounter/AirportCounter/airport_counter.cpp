...
public:
	...
	size_t Get(TAirport airport) const {
		return airports_counts[static_cast<size_t>(airport)];
	}
	...
	Items GetItems() const {
		...
		for (size_t i = 0; i < N; i++) {
			items[i] = { static_cast<TAirport>(i), airports_counts[i] };
		}
...
private:
	array<size_t, N> airports_counts;

	size_t& Ref(TAirport airport) {
		return airports_counts[static_cast<size_t>(airport)];
	}
};
...
void TestManyConstructions() {
	LOG_DURATION("many constructions");
