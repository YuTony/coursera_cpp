...
class ConcurrentMap {
    struct Page {  // у каждой страницы свой mutex и словарь
        mutex page_mutex;
        map<K, V> page_map;
    };
   ...
   public:
    ...
    Access operator[](const K& key) {    // key будет приведен к unsigned int, но на результат % это не повлияет (зато не нужно вызывать abs())
    ...
    map<K, V> BuildOrdinaryMap() {  // вернуть сложенный из страниц словарь
        map<K, V> result;
        for (auto& page : data) {
