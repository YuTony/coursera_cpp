...
class AirportCounter {
   public:
    ...
    AirportCounter(TIterator begin, TIterator end) : AirportCounter() {
        TIterator it = begin;
        while (it != end) {
            db[static_cast<size_t>(*it++)] += 1;
        }
    }

    // получить количество элементов, равных данному
    size_t Get(TAirport airport) const {
        return db[static_cast<size_t>(airport)];
    }

    // добавить данный элемент
    void Insert(TAirport airport) {
        db[static_cast<size_t>(airport)]++;
    }

    // удалить одно вхождение данного элемента
    void EraseOne(TAirport airport) {
        if (db[static_cast<size_t>(airport)] != 0) {
            db[static_cast<size_t>(airport)]--;
        }
    }
    ...
    void EraseAll(TAirport airport) {
        if (db[static_cast<size_t>(airport)] != 0) {
            db[static_cast<size_t>(airport)] = 0;
        }
    }
    ...
    Items GetItems() const {
        ...
        for (size_t i = 0; i < db.size(); i++) {
            result[i] = {static_cast<TAirport>(i), db[i]};
        }
        return result;
    }

   private:
    array<size_t, static_cast<uint32_t>(TAirport::Last_)> db;
};
